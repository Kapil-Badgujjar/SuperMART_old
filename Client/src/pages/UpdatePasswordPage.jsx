...
export default function UpdatePassword() {
  ...
  useEffect(() => {
    async function fun()
   { const {status} = await axios.get(`https://super-mart-backend.vercel.app/user/activatepasswordreset/${token}`);
    if(status !== 200){
        navigate('/');
    }
      setLoading(false);}
  ...
  function update(event) {
    ...
    async function Fun(){
      const {status,data} = await axios.post(`https://super-mart-backend.vercel.app/user/resetpassword`, {password: newPassword }, {withCredentials: true});
      if(status === 200){
